...
const ManufacturerSelect = ({ value, onChange, options, error }) => {
      ...
      >
        {selected ? (
          <div className='flex items-center gap-3'>
            {selected.logoUrl && (
              <img
                src={`${import.meta.env.VITE_BACKEND_URL}${selected.logoUrl}`}
                alt=''
          ...
          <span className='text-gray-500'>Chọn nhà sản xuất</span>
        )}
        {isOpen ? (
        ...
        <div className='absolute z-50 w-full mt-1 bg-white dark:bg-gray-700 border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-y-auto'>
          {options.map((option) => (
            ...
            >
              {option.logoUrl && (
                <img
                  src={`${import.meta.env.VITE_BACKEND_URL}${option.logoUrl}`}
                  alt=''
                  className='w-8 h-8 object-contain rounded-full'
                  onError={(e) => (e.target.style.display = 'none')}
                />
